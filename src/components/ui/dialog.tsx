...
function DialogContent({
  ...
  return (
    <DialogPortal>
      ...
      <div
        className={cn(
          "fixed inset-x-0 bottom-0 top-0 z-50 flex justify-center pointer-events-none w-full max-w-md mx-auto px-4",
          isBottom ? "items-end pb-24" : "items-center"
        )}
      >
        <DialogPrimitive.Popup
          data-slot="dialog-content"
          className={cn(
            "relative pointer-events-auto grid w-full gap-4 rounded-xl bg-popover p-4 text-sm text-popover-foreground ring-1 ring-foreground/10 duration-100 outline-none sm:max-w-sm data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
            className
          )}
          {...props}
        >
